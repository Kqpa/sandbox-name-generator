...
int main(void) {
    ...
    if (!PLAIN_OUTPUT) {
        ...
        output << "The avalible prefixes, declinations & suffixes are listed below. You can add and/or remove them by editing the 'include/config/vectors.hpp' file.\n";
        output << "[PREFIXES]: ";
    ...
    if (NAME_GENERATION_COUNT == DEFAULT_NAME_COUNT) {
        for (size_t i = 0; i < NAMES_ARR_SIZE; ++i) {
            if (PLAIN_OUTPUT) {
                ...
                output << "[#" << i + 1 << "]: New name: '" << names.at(i) << "'\n";
            }
        }
        ...
        for (size_t i = 0; i < NAME_GENERATION_COUNT; ++i) {
            if (PLAIN_OUTPUT) {
                ...
                output << "[#" << i + 1 << "]: New name: '" << names.at(better_rand(0, NAMES_ARR_SIZE - 1)) << "'\n";
            }
        }
    } else {
